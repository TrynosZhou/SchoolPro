...
async function generateReportCardPdf(data) {
    return new Promise((resolve, reject) => {
        ...
        const attendanceRate = data.attendance?.attendancePercent != null ? `${data.attendance.attendancePercent}%` : '100%';
        ...
        doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(11).text('ACADEMIC PERFORMANCE BY SUBJECT', tableX + 12, y + 10, { width: tableW - 24, lineBreak: false, characterSpacing: 0.4 });
        ...
        let ry = remarksY + 18;
        const drawRemarkBlock = (title, text) => {
            const labelW = 190;
