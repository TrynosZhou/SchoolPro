...
exports.generateStudentIdCardPdf = generateStudentIdCardPdf;
...
function drawPdfBankingDetails(doc, branding, margin, contentW, y, pageBottom, onNewPage) {
    const hasBankDetails = branding.bankAccountName || branding.bankName || branding.bankBranch || branding.bankAccountNumber;
    if (!hasBankDetails)
        return y;
    ...
    y += 18;
    const bankRows = [];
    if (branding.bankAccountName)
        bankRows.push({ label: 'Account name', value: branding.bankAccountName });
    if (branding.bankName)
        bankRows.push({ label: 'Bank', value: branding.bankName });
    if (branding.bankBranch)
        bankRows.push({ label: 'Branch', value: branding.bankBranch });
    if (branding.bankAccountNumber)
        bankRows.push({ label: 'Account number', value: branding.bankAccountNumber });
    const bankRowH = 20;
...
function drawPdfPaymentReferenceNote(doc, note, margin, contentW, y, pageBottom) {
    const paymentNote = note?.trim() || 'Please use the account number as your payment reference.';
    if (y + 20 >= pageBottom || !paymentNote)
        return y;
...
async function generateReceiptPdf(data) {
    ...
    const qrBuffer = await qrcode_1.default.toBuffer(verifyPayload, { margin: 1, width: 200, errorCorrectionLevel: 'M' });
    return new Promise((resolve, reject) => {
        ...
        const contactParts = [];
        if (data.address)
            contactParts.push(data.address);
        if (data.phone)
            contactParts.push(`Tel: ${data.phone}`);
        if (data.email)
...
async function generateInvoicePdf(data) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const summaryW = 240;
        const summaryX = margin + contentW - summaryW;
        const drawSummaryRow = (label, value, highlight = false, isTotal = false) => {
...
async function generateReportCardPdf(data) {
    return (0, report_card_portal_pdf_1.generateReportCardPortalPdf)(data);
}
...
function drawHorizontalGradient(doc, x, y, w, h, leftHex, rightHex) {
...
function drawBarcodeStrip(doc, x, y, w, h, payload) {
...
async function generateStudentIdCardPdf(data) {
    return new Promise((resolve, reject) => {
        ...
        const rightX = leftW;
        const doc = new pdfkit_1.default({ size: [cardW, cardH], margin: 0 });
        const chunks = [];
        doc.on('data', (c) => chunks.push(c));
        doc.on('end', () => resolve(Buffer.concat(chunks)));
        doc.on('error', reject);
