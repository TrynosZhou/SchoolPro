...
exports.PAYMENT_METHOD_GROUPS = exports.FEE_CATEGORY_LABELS = void 0;
exports.buildFeeCollectionRevenueReport = buildFeeCollectionRevenueReport;
exports.feeCollectionReportToCsv = feeCollectionReportToCsv;
const data_source_1 = require("../config/data-source");
const entities_1 = require("../entities");
const enums_1 = require("../entities/enums");
const fin_reports_service_1 = require("./fin-reports.service");
const EPS = 0.005;
exports.FEE_CATEGORY_LABELS = {
...
exports.PAYMENT_METHOD_GROUPS = [
    { key: 'cash', label: 'Cash', methods: [enums_1.PaymentMethod.CASH] },
    { key: 'bank', label: 'Bank transfer / deposit', methods: [enums_1.PaymentMethod.BANK] },
    {
        key: 'mobile',
        label: 'Mobile money',
        methods: [enums_1.PaymentMethod.ECOCASH, enums_1.PaymentMethod.ONEMONEY, enums_1.PaymentMethod.INNBUCKS],
    },
    { key: 'cheque', label: 'Cheque', methods: [] },
    { key: 'online', label: 'Online payment', methods: [enums_1.PaymentMethod.OTHER] },
];
...
function feeLabel(feeType) {
    return exports.FEE_CATEGORY_LABELS[feeType] || feeType.replace(/_/g, ' ');
}
function methodLabel(method) {
    ...
    if (['ecocash', 'onemoney', 'innbucks'].includes(m))
        return 'Mobile money';
    if (m === 'other')
        return 'Online / other';
    return m;
}
function paymentMatchesMethodFilter(method, filter) {
    if (!filter)
        return true;
    const m = String(method || '').toLowerCase();
    ...
    if (f === 'bank')
        return m === 'bank';
    if (f === 'mobile')
        return ['ecocash', 'onemoney', 'innbucks'].includes(m);
    if (f === 'cheque')
...
function isReversedPayment(notes) {
    const n = String(notes || '').toLowerCase();
    return /revers|cancel|refund|void/.test(n);
}
async function resolvePeriodDates(dateFrom, dateTo, termId) {
    if (termId) {
...
async function buildFeeCollectionRevenueReport(params) {
    ...
    if (!studentId && params.q) {
        const matches = await (0, fin_reports_service_1.searchStudents)(params.q, 20);
        if (!matches.length)
    ...
    const invQb = invoiceRepo
        ...
        .andWhere('i.status != :cancelled', { cancelled: enums_1.InvoiceStatus.CANCELLED });
    if (params.termId) {
        invQb.andWhere('i.termId = :termId', { termId: params.termId });
    }
    else {
        invQb.andWhere('COALESCE(i."issuedDate", i."dueDate") >= :dateFrom AND COALESCE(i."issuedDate", i."dueDate") <= :dateTo', { dateFrom: period.dateFrom, dateTo: period.dateTo });
    }
    if (params.feeType)
        invQb.andWhere('i.feeType = :feeType', { feeType: params.feeType });
    const invoices = await invQb.getMany();
    const paymentRows = await data_source_1.AppDataSource.query(`
      SELECT
        p.id as "paymentId",
        p."paidAt",
        p.amount,
    ...
    const filteredPayments = paymentRows.filter((p) => paymentMatchesMethodFilter(String(p.method), params.paymentMethod || ''));
    ...
    const paymentCollectedByStudent = new Map();
    for (const p of filteredPayments) {
        if (isReversedPayment(p.notes))
            continue;
    ...
    for (const [sid, payTotal] of paymentCollectedByStudent) {
        ...
        if (agg)
            agg.collected = Math.max(agg.collected, payTotal);
    }
    ...
    for (const inv of invoices) {
        if (!allowedStudentIds.has(inv.studentId))
            continue;
        ...
        if (inv.status === enums_1.InvoiceStatus.CANCELLED) {
    ...
    for (const p of filteredPayments) {
        if (!allowedStudentIds.has(String(p.studentId)))
            continue;
        const date = toDateKey(p.paidAt);
        const st = studentMap.get(String(p.studentId));
        if (!st)
            continue;
        const amt = Number(p.amount);
        const reversed = isReversedPayment(p.notes);
        ...
        if (!reversed)
            day.dayTotal = roundMoney(day.dayTotal + amt);
        else {
            day.reversedCount += 1;
            exceptions.push({
                type: 'reversed',
        ...
        if (mKey === 'cash')
            ...
            groupKey = 'bank';
        else if (['ecocash', 'onemoney', 'innbucks'].includes(mKey))
            groupKey = 'mobile';
    ...
    for (const d of daily)
        d.dayTotal = roundMoney(d.dayTotal);
    const weekMap = new Map();
    let cumulativeCollected = 0;
    let cumulativeExpected = 0;
    const addToWeek = (dateStr, expected, collected) => {
        const wk = isoWeekKey(dateStr);
        const row = weekMap.get(wk.key) ||
            {
                weekKey: wk.key,
                weekLabel: wk.label,
                weekStart: wk.start,
    ...
    for (const p of filteredPayments) {
        ...
        if (isReversedPayment(p.notes))
            continue;
        addToWeek(toDateKey(p.paidAt), 0, Number(p.amount));
    }
    ...
    for (const p of filteredPayments) {
        ...
        if (isReversedPayment(p.notes))
            continue;
        const mk = toDateKey(p.paidAt).slice(0, 7);
        const m = monthMap.get(mk) ||
            {
                monthKey: mk,
                monthLabel: mk,
    ...
    const paymentMethods = exports.PAYMENT_METHOD_GROUPS.map((g) => {
    ...
    const dailyCollections = daily.map((d) => ({
        label: d.date,
        value: d.dayTotal,
    }));
    let runCollected = 0;
    ...
    const feeTypeBreakdown = [...categoryMap.values()]
        .filter((c) => c.collected > 0)
        .map((c) => ({ label: c.label, value: c.collected }));
    const gradeClassRates = byGradeClass.slice(0, 12).map((g) => ({
    ...
    const report = {
        ...
        auditNote: 'All payment transactions are recorded with cashier and receipt. Reversals and write-offs above threshold require dual authorization per school policy.',
    };
    return report;
}
function feeCollectionReportToCsv(report, detailed) {
    const esc = (v) => {
        const s = String(v ?? '');
    ...
    if (detailed) {
        ...
        lines.push('Date,Student ID,Name,Class,Fee Type,Expected,Paid,Method,Receipt,Cashier,Outstanding After,Reversed');
