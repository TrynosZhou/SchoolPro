...
import { getActiveSchoolYear } from './student-lifecycle.service';
...
function studentTypeLabel(raw?: string | null): 'Boarder' | 'Day Scholar' {
  return (raw || '').toLowerCase() === 'boarder' ? 'Boarder' : 'Day Scholar';
}

function ageFromDob(dob?: string | null, asOf?: string | null): number | null {
  if (!dob) return null;
...
const AGE_BANDS: { key: string; min: number; max: number }[] = [
...
async function resolveSchoolYearId(filters: {
  ...
  if (filters.schoolYearId) return filters.schoolYearId;
  if (filters.termId) {
    const term = await AppDataSource.getRepository(Term).findOne({ where: { id: filters.termId } });
...
function uuidWhere(column: string, index: number): string {
  return `${column} = $${index}::uuid`;
}

/** Text comparison for snapshot columns stored as varchar (legacy sync). */
function textIdWhere(column: string, index: number): string {
  return `${column} = $${index}`;
}
...
export async function getDemographics(filters: DemographicsFilters) {
  const schoolYearId = await resolveSchoolYearId(filters);
  const current = await getActiveSchoolYear();
  ...
  if (schoolYearId) {
    ...
    const snapCount = await AppDataSource.query(
      `SELECT COUNT(*)::int AS c FROM student_enrollments WHERE ${uuidWhere('"schoolYearId"', 1)}`,
      [schoolYearId],
    );
    if (Number(snapCount?.[0]?.c || 0) > 0) source = 'snapshot';
  }

  if (source === 'snapshot' && schoolYearId) {
    const params: string[] = [schoolYearId];
  ...
  const formMap = new Map<string, Grp>();
  const classMap = new Map<string, Grp>();

  const bump = (map: Map<string, Grp>, id: string, label: string, level: number, g: GenderKey, isBoarder: boolean) => {
    let grp = map.get(id);
    if (!grp) {
    ...
    grp.total++;
    if (g === 'Male') grp.male++;
    else if (g === 'Female') grp.female++;
    else grp.unspecified++;
    if (isBoarder) grp.boarders++;
    else grp.dayScholars++;
  };
  ...
  for (const r of rows) {
    ...
    else if (g === 'Female') totals.female++;
    else totals.unspecified++;
    if (isBoarder) totals.boarders++;
    else totals.dayScholars++;
  ...
  const byGender: GenderKey[] = ['Male', 'Female', 'Unspecified'];
  return {
    source,
    ...
    byAge: AGE_BANDS.map((b) => ({ key: b.key, count: ageMap.get(b.key) || 0 })).concat(
      ageMap.get('Unknown') ? [{ key: 'Unknown', count: ageMap.get('Unknown') || 0 }] : [],
    ),
  };
}
...
export async function getRetention() {
  ...
  const yearOrder: { id: string; name: string; startDate: string }[] = [];
  const seenYear = new Set<string>();
  const yearStudents = new Map<string, Set<string>>();
  const yearGraduates = new Map<string, Set<string>>();
  ...
  for (const e of enrollments) {
    ...
    if (e.status === 'completed') yearGraduates.get(e.schoolYearId)!.add(e.studentId);
  }
...
// ---------------------------------------------------------------------------
// 3. At-risk students (attendance + performance + fees)
// ---------------------------------------------------------------------------
...
export async function getAtRiskStudents(filters: AtRiskFilters) {
  // Determine the analysis window from the selected/current term (fallback: last 90 days).
  let term: Term | null = null;
  if (filters.termId) {
    term = await AppDataSource.getRepository(Term).findOne({ where: { id: filters.termId } });
  }
  if (!term) {
    term = await AppDataSource.getRepository(Term).findOne({ where: { isCurrent: true } });
  }
  ...
  `);
  const perfMap = new Map<string, { latest?: number; prev?: number; passed?: number; total?: number }>();
  ...
  const feeRows: { studentId: string; owed: number }[] = await AppDataSource.query(`
  ...
  for (const f of feeRows) feeMap.set(f.studentId, Number(f.owed || 0));
  ...
  for (const a of attendance) {
    ...
    const trend =
      perf.latest !== undefined && perf.prev !== undefined
