...
import { normalizeGender } from './analytics.service';
import type { ReportTemplateConfig } from '../entities/ReportTemplate';

// ---------------------------------------------------------------------------
// Field & dataset registry
// ---------------------------------------------------------------------------

export type FieldType = 'string' | 'number' | 'money' | 'percent' | 'date';
...
export interface DatasetMeta {
  ...
  description: string;
  fields: FieldDef[];
  /** Filter ids this dataset understands. */
  filters: string[];
  defaultFields: string[];
  /** Field keys that make sense to group by. */
  groupable: string[];
}
...
export interface FilterMeta {
  id: string;
  label: string;
  type: 'class' | 'form' | 'status' | 'schoolYear' | 'term' | 'date';
}
...
export function getReportMeta() {
  return { datasets: DATASETS, filters: FILTERS };
}

function datasetByKey(key: string): DatasetMeta | undefined {
  return DATASETS.find((d) => d.key === key);
}

// ---------------------------------------------------------------------------
// Data fetching per dataset
// ---------------------------------------------------------------------------
...
async function fetchStudents(filters: Filters): Promise<Row[]> {
  const params: string[] = [];
  let where = '';
  ...
  return rows.map((r) => {
    ...
    if (dob && !Number.isNaN(dob.getTime())) {
      ...
      if (m < 0 || (m === 0 && todayIso.getDate() < dob.getDate())) age--;
    }
    return {
      ...r,
      genderLabel: normalizeGender(r.gender as string),
...
async function resolveDateRange(filters: Filters): Promise<{ start: string; end: string }> {
  const from = clean(filters.dateFrom);
  const to = clean(filters.dateTo);
  if (from && to) return { start: from, end: to };
  let term: Term | null = null;
  if (clean(filters.termId)) {
    term = await AppDataSource.getRepository(Term).findOne({ where: { id: clean(filters.termId)! } });
  }
  if (!term) term = await AppDataSource.getRepository(Term).findOne({ where: { isCurrent: true } });
...
async function fetchAttendance(filters: Filters): Promise<Row[]> {
...
async function fetchGrades(filters: Filters): Promise<Row[]> {
  const params: string[] = [];
...
async function fetchFees(filters: Filters): Promise<Row[]> {
  const params: string[] = [];
...
async function fetchRows(datasetKey: string, filters: Filters): Promise<Row[]> {
  switch (datasetKey) {
    case 'students':
...
export async function runReport(config: ReportTemplateConfig): Promise<ReportResult> {
  const ds = datasetByKey(config.dataset);
  if (!ds) throw new Error(`Unknown dataset: ${config.dataset}`);

  const validFieldKeys = new Set(ds.fields.map((f) => f.key));
  let selectedKeys = (config.fields || []).filter((k) => validFieldKeys.has(k));
  if (!selectedKeys.length) selectedKeys = ds.defaultFields.slice();

  const rows = await fetchRows(ds.key, config.filters || {});
  const groupBy =
    config.groupBy && ds.groupable.includes(config.groupBy) ? config.groupBy : null;

  let outRows: Row[];
  let columns: FieldDef[];

  if (groupBy) {
    const groupDef =
      ds.fields.find((f) => f.key === groupBy) ||
      ({ key: groupBy, label: groupBy, type: 'string' } as FieldDef);
    const numericSelected = selectedKeys
      .map((k) => ds.fields.find((f) => f.key === k))
      .filter(
        (f): f is FieldDef =>
          !!f && f.key !== groupBy && (f.type === 'number' || f.type === 'money' || f.type === 'percent'),
      );

    const groups = new Map<string, { rows: Row[] }>();
    for (const r of rows) {
      const gv = r[groupBy] != null && r[groupBy] !== '' ? String(r[groupBy]) : 'Unassigned';
      if (!groups.has(gv)) groups.set(gv, { rows: [] });
      groups.get(gv)!.rows.push(r);
    }

    outRows = [...groups.entries()].map(([gv, g]) => {
      const row: Row = { [groupBy]: gv, __count: g.rows.length };
...
export async function reportToXlsx(result: ReportResult, title?: string): Promise<Buffer> {
  ...
  result.rows.forEach((row) => {
    ...
    result.columns.forEach((c, i) => {
      if (c.type === 'money') added.getCell(i + 1).numFmt = '#,##0.00';
      if (c.type === 'percent') added.getCell(i + 1).numFmt = '0.0"%"';
    });
  });
...
export async function reportToPdf(result: ReportResult, title: string): Promise<Buffer> {
  ...
  return new Promise<Buffer>((resolve, reject) => {
    ...
    doc.text(
      `Generated ${new Date(result.generatedAt).toLocaleString()}  •  ${result.totalRows} row(s)` +
