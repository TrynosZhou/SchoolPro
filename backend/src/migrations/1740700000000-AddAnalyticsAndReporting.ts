 ...
 */
export class AddAnalyticsAndReporting1740700000000 implements MigrationInterface {
  name = 'AddAnalyticsAndReporting1740700000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    // 1. Student lifecycle columns.
    await queryRunner.query(
      `ALTER TABLE "students" ADD COLUMN IF NOT EXISTS "status" varchar(24) NOT NULL DEFAULT 'active'`,
    );
    await queryRunner.query(`ALTER TABLE "students" ADD COLUMN IF NOT EXISTS "exitDate" date`);
    await queryRunner.query(
      `ALTER TABLE "students" ADD COLUMN IF NOT EXISTS "exitReason" varchar(255)`,
    );
    await queryRunner.query(
      `ALTER TABLE "students" ADD COLUMN IF NOT EXISTS "updatedAt" TIMESTAMP NOT NULL DEFAULT now()`,
    );

    // Backfill: inactive students that were never given a specific exit status become 'withdrawn'.
    await queryRunner.query(
      `UPDATE "students" SET "status" = 'withdrawn' WHERE "isActive" = false AND ("status" IS NULL OR "status" = 'active')`,
    );
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_enrollment_student" ON "student_enrollments" ("studentId")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_enrollment_year" ON "student_enrollments" ("schoolYearId")`,
    );
