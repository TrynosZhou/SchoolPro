...
export class AddCommunicationFeatures1740600000000 implements MigrationInterface {
  name = 'AddCommunicationFeatures1740600000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    // 1. Threaded conversation key on messages.
    await queryRunner.query(
      `ALTER TABLE "messages" ADD COLUMN IF NOT EXISTS "threadId" varchar(128)`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "school_settings" ADD COLUMN IF NOT EXISTS "notificationSettings" jsonb`,
    );
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_bulk_recipients_messageId" ON "bulk_message_recipients" ("bulkMessageId")`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "school_settings" DROP COLUMN IF EXISTS "notificationSettings"`,
    );
