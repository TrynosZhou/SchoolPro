...
export function shortClassName(className?: string | null): string {
...
export interface TeacherLoadPdfRow {
  ...
  /** Comma-separated class names, e.g. 1A,1B,2A */
  classesLabel: string;
  /** e.g. 5*3 when each class has the same weekly load */
  periodsFormula: string;
  totalPeriods: number;
}

export interface TeacherLoadPdfData {
  schoolName: string;
  tagline?: string;
  logoUrl?: string;
  generatedAt: Date;
  ...
  rows: TeacherLoadPdfRow[];
}

export function buildPeriodsFormula(classLoads: number[]): string {
...
interface TeacherLoadPdfClassInput {
  className: string;
  classLoad: number;
  subjects?: {
...
function lessonLengthFactor(lessonLength?: string): number {
  if (lessonLength === 'double') return 2;
  if (lessonLength === 'triple') return 3;
  return 1;
}

/** Build formula like 5*3 (single) or 5*2*3 (double lessons across 3 classes). */
export function buildTeacherPeriodsFormula(classes: TeacherLoadPdfClassInput[]): string {
  ...
  if (
    patterns.every(
      (p) =>
        p.weeklyPeriods === first.weeklyPeriods &&
...
export function mapTeacherLoadReportToPdfRows(
  teachers: {
    firstName: string;
    lastName: string;
    employeeNumber: string;
    totalLoad: number;
    classes: TeacherLoadPdfClassInput[];
  }[],
): TeacherLoadPdfRow[] {
...
export async function generateTeacherLoadPdf(data: TeacherLoadPdfData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.text(
      `${data.summary.teacherCount} teachers · ${data.summary.teachersWithAssignments} with assignments · ${data.summary.totalPeriods} total periods / week`,
      margin,
      y,
      { width: contentW },
    );
    ...
    const cols = [
      { label: 'TEACHER', w: 140, align: 'left' as const },
      { label: 'EMP ID', w: 72, align: 'left' as const },
      { label: 'CLASSES', w: 200, align: 'left' as const },
      { label: 'PERIODS', w: 80, align: 'center' as const },
      { label: 'TOTAL PERIODS', w: contentW - 140 - 72 - 200 - 80, align: 'center' as const },
    ];
