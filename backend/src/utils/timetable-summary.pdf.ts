...
import { teacherInitialsFromDisplayName } from './teacher-display';
...
export interface SummaryGridPeriod {
  startTime: string;
  endTime: string;
  name?: string;
...
export interface SummaryGridSlot {
  dayOfWeek: number;
  startTime: string;
  endTime: string;
  className: string;
...
export interface SummaryGridTeacher {
  teacherLabel: string;
  teacherName: string;
  employeeNumber?: string;
  slots: SummaryGridSlot[];
}

export interface TimetableSummaryPdfData {
  schoolName: string;
  tagline?: string;
  logoUrl?: string;
  generatedAt: Date;
  periods: SummaryGridPeriod[];
  teachers: SummaryGridTeacher[];
}
...
export function teacherInitials(name: string): string {
  return teacherInitialsFromDisplayName(name) || '?';
}

export function summaryGridCell(
  teacher: SummaryGridTeacher,
  dayOfWeek: number,
  period: SummaryGridPeriod,
): string {
  if (period.slotType === 'break') return '';
  const matches = teacher.slots.filter(
    (s) => s.dayOfWeek === dayOfWeek && s.startTime === period.startTime && s.endTime === period.endTime,
  );
  if (!matches.length) return '';
  return [...new Set(matches.map((s) => shortClassName(s.className)).filter(Boolean))].join('/');
}

function isBreakPeriod(period: SummaryGridPeriod): boolean {
...
function breakPeriodLabel(period: SummaryGridPeriod): string {
  ...
  if (/lunch/i.test(name)) return 'Ln';
  if (/mid/i.test(name)) return 'MM';
  if (/morning/i.test(name)) return 'MB';
  return 'Br';
}

function periodHeaderLabel(periods: SummaryGridPeriod[], index: number): string {
  if (isBreakPeriod(periods[index])) return breakPeriodLabel(periods[index]);
  return String(lessonPeriodNumber(periods, index));
}

function formatPeriodRangeCompact(period: SummaryGridPeriod): string {
...
function buildPeriodLegend(periods: SummaryGridPeriod[]): string {
...
export async function generateTimetableSummaryPdf(data: TimetableSummaryPdfData): Promise<Buffer> {
...
export interface ClassesGridSlot {
  dayOfWeek: number;
  startTime: string;
  endTime: string;
  subjectName: string;
...
export interface ClassesGridRow {
  classLabel: string;
  className: string;
  slots: ClassesGridSlot[];
}

export interface TimetableClassesGridPdfData {
  schoolName: string;
  tagline?: string;
  logoUrl?: string;
  generatedAt: Date;
  periods: SummaryGridPeriod[];
  classes: ClassesGridRow[];
}

function classesGridCell(
  cls: ClassesGridRow,
  dayOfWeek: number,
  period: SummaryGridPeriod,
): string {
  if (period.slotType === 'break') return '';
  const matches = cls.slots.filter(
    (s) => s.dayOfWeek === dayOfWeek && s.startTime === period.startTime && s.endTime === period.endTime,
  );
  if (!matches.length) return '';
...
export async function generateTimetableClassesGridPdf(data: TimetableClassesGridPdfData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const margin = 28;
    const doc = new PDFDocument({ margin, size: 'A4', layout: 'landscape' });
    const chunks: Buffer[] = [];
