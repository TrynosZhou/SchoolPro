...
export function formatStudentClassLabel(className?: string | null): string {
  ...
  if (!name) return '—';
  if (/^class\s+/i.test(name)) return name;
  return `Class ${name}`;
}
...
export function isALevelForm(form?: { name?: string | null; level?: number | null } | null): boolean {
...
export function isALevelClassOption(
  cls?: { name?: string | null; form?: { name?: string | null; level?: number | null } } | null,
): boolean {
  if (!cls) return false;
