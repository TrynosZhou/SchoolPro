  ...
  if (!bootPromise) {
    ...
    bootPromise = initializeServer()
      .then(() => {
        ...
        console.log(
          `[api:root] boot finished — ok=${state.ok} startedAt=${state.startedAt}`,
          state.error
      ...
      .catch((err) => {
        ...
        console.error(
          '[api:root] initializeServer threw (continuing in degraded mode) — ' +
...
module.exports = async (req, res) => {
  try {
    ...
    console.error('[api:root] bootOnce unexpected catch path (should be swallowed).', err && err.message);
  }
  try {
